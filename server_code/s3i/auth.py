...
class BaseAuthenticator:
    def __init__(self, client: httpx.AsyncClient = None, idp_url: str = DEFAULT_IDP_URL):
    ...
    async def _get_token_from_idp(self) -> Token:
        ...
        if response.status_code >= 400:
            if response.text == '{"error":"invalid_client","error_description":"Invalid client credentials"}':
                raise InvalidCredentialsException("Invalid client credentials.", status_code=response.status_code,
    ...
    async def _refresh_token(self) -> Token:
        ...
        if response.status_code >= 400:
            raise AuthenticationException("Could not refresh token from S³I Identity Provider.",
                                          status_code=response.status_code,
                                          response=response.text)
...
class ClientAuthenticator(BaseAuthenticator):
    ...
    def __init__(self, id: str, secret: str, client: httpx.AsyncClient = None, idp_url: str = DEFAULT_IDP_URL):
...
class PasswordAuthenticator(BaseAuthenticator):
    ...
    def __init__(self, id: str, secret: str, username: str, password: str, client: httpx.AsyncClient = None,
